...
### 方式 A-补充：你也可以手工下载遥感影像，不用先写命令

如果你现在还不想用命令下载，完全可以自己去平台上手工下载遥感图。
...
如果平台上让你选很多参数，你先只抓这几个：

- 区域：先画一个小矩形
- 时间：先选一个年份范围
- 云量：尽量低
- 波段：如果你只是先做基础水面分割，优先 RGB 或常用可见光波段
- 导出格式：优先 `GeoTIFF`
...
```text
jingshi_manual_001.tif
```

就直接放到：

```text
/root/SeeHydro/raw_geotiff/jingshi_manual_001.tif
```
...
1. 检查这张图有没有 `crs/transform/bounds`
2. 确认图能正常打开
3. 再做切片
4. 再从切片里挑少量样本去标注
